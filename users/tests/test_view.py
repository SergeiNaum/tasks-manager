...
class TestListUsers(TestCase):
    def setUp(self):
        ...
        self.credentials2 = {"username": "test_user2", "password": "te$t_pa$$word2"}

        self.credentials3 = {"username": "test_user3", "password": "te$t_pa$$word3"}
    ...
    def test_users_content(self) -> None:
        ...
        self.assertQuerysetEqual(response.context["users"], self.users, ordered=False)
...
class TestUpdateUserView(TestCase):
    ...
    def test_update_self_view(self) -> None:
        ...
        response = self.client.get(reverse_lazy("users:edit_user", kwargs={"pk": 2}))

        self.assertEqual(response.status_code, HTTPStatus.OK)
        self.assertTemplateUsed(
            response, template_name="users/edit_user_form.html"
        )  # noqa E501
